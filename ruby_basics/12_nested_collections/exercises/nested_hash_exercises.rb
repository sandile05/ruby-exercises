...
def find_beautiful_languages(languages)
  ...
  langs = languages.select { |k, v| v[:is_beautiful?] == true }
  langs
end
...
def find_language_facts(languages, language_name, fact_index = 0)
  ...
  # Return the fact at the specified fact_index or the first fact if fact_index is not provided
  fact = language[:facts][fact_index] || language[:facts][0]
  fact
end
